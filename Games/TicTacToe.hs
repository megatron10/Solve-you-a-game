...
data TTTState = TTTState
  { board  :: TTTBoard -- ^ Represents Cell contents of board
...
setSymbol posn symb cboard = map modify $ zip [1 ..] cboard
  where
    modify (i, s)
      | i == posn = symb
      | otherwise = s
...
tttDoMove (TTTState cboard PlayerOne) i =
  TTTState (setSymbol i X cboard) PlayerTwo
tttDoMove (TTTState cboard PlayerTwo) i =
  TTTState (setSymbol i O cboard) PlayerOne
...
getDiag2 b = map snd $ filter isD2 (zip [1 ..] b)
  where
    isD2 (n, _) =
      n `rem` (boardSize - 1) == 1 && n > 1 && n < (boardSize * boardSize)
...
tttBaseCase (TTTState cboard _)
  ...
  | otherwise = Undecided
  where
    winLine_X = replicate boardSize X
...
boardToString b = intercalate horizLine rows
  where
    horizLine = (replicate (4 * boardSize - 1) '-') ++ "\n"
